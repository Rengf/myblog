<template>
  <div class="pages" v-if="dataTotal>0">
    <nav>
      <ul class="pagination">
...
export default {
  ...
  computed: {
    pageTotal() {
      // 总页数
      if (this.display == "") {
        return Math.ceil(this.dataTotal / 5);
      }
  ...
  methods: {
    //跳到某一页
    setCurrent(index) {
      console.log(index, this.pageTotal);
      if (index > this.pageTotal || index < 0) {
        return;
      }
